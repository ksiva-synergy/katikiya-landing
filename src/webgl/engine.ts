...
export class Engine {
  ...
  mount(root: ParentNode = document) {
    ...
    root.querySelectorAll<HTMLCanvasElement>('canvas[data-viz]').forEach((canvas) => {
      ...
      canvas.addEventListener('pointerup', pl)
      canvas.addEventListener('pointercancel', pl)
      this.cleanups.push(() => {
        window.removeEventListener('resize', fit)
        canvas.removeEventListener('pointermove', pm)
        canvas.removeEventListener('pointerleave', pl)
        canvas.removeEventListener('pointerup', pl)
        canvas.removeEventListener('pointercancel', pl)
        renderer.dispose()
      })
